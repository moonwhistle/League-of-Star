...
- 랭킹 요약 영역은 이번 PR의 source of truth로 다루지 않음.
  `내 순위 #128`, `상위 7%`, `시즌 최고 #94`는 랭킹 API 계약이 필요한 영역이므로 이번 이슈에서 실데이터로 해석하지 않음.

## 📝 Note

- 랭킹 리스트 전체 실데이터 전환은 후속 2-4 이후 진행함.
- `내 순위`, `상위`, `시즌 최고` 요약 영역 실데이터 전환도 후속 2-4/2-5 범위임.
- 현재 유저 row는 profile/rank 응답으로 nickname/LP만 보정하며, 실제 순위 값은 후속 랭킹 API 전까지 `-`로 둠.
