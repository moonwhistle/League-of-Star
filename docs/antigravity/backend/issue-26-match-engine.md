...
매칭 V1은 `joinQueue` 요청이 들어올 때마다 즉시 매칭을 시도하는 이벤트 기반 방식이 아니라, Redis 대기열을 주기적으로 스캔하는 배치형 매칭 엔진으로 구현했습니다.

매칭 정책상 오래 기다린 유저가 우선권을 가져야 하므로, 개별 이벤트가 들어온 순서대로 부분 매칭을 수행하는 방식보다 전체 대기열을 한 번에 로드한 뒤 `entryTime` 기준으로 정렬하는 방식이 더 적합하다고 판단했습니다. 이를 통해 모든 티어의 대기열을 통합 관점에서 보고, 가장 오래 기다린 유저부터 슬라이딩 윈도우 정책에 따라 매칭 후보를 찾을 수 있습니다.
...
- 매칭 엔진 실행을 `MatchEngine`, `MatchEngineService`, `MatchFoundProcessor`로 분리했습니다.
...
- 멀티 인스턴스 중복 스캔 방지를 위해 Redisson 전역 락을 적용했습니다.
  ...
  - LeaseTime은 스케줄 간격보다 길게 설정해 엔진 장애 시에도 락이 영구 고립되지 않게 했습니다.
...
- 이벤트 기반 매칭은 `joinQueue` 요청 직후 빠르게 반응할 수 있지만, 요청이 발생한 일부 구간만 보고 매칭을 시도하기 쉽습니다. V1 정책은 오래 기다린 유저 우선 매칭이 핵심이므로, 전체 대기열을 주기적으로 보고 FIFO 순서를 보장하는 배치형 엔진을 선택했습니다.
...
- [ ] **부하 테스트 시나리오 작성**
  - Scenario A: 지속 유입 테스트 (`docs/load-test/join-queue-steady.js`)
    ...
    - 5,000명 / 3분
    - 10,000명 / 3분
    - 목표: 운영 중 꾸준한 매칭 진입 요청에서 API 응답 시간, 큐 적체, 매칭 대기 시간 p95를 확인
