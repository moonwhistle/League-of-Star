...
이번 이슈의 목표는 **매칭 성사 이벤트를 클라이언트까지 안정적으로 전달하는 실시간 이벤트 스트림 구조를 만드는 것**입니다. 클라이언트는 매칭 시작 요청을 보내기 전에 SSE 연결을 열고, 서버는 `MatchFoundEvent`가 발생하면 해당 유저의 SSE 연결로 `match_found` 이벤트를 전송합니다.
...
- 매칭 시작 요청 이후 대기/응답 흐름 동안만 연결을 유지하면 되므로 연결 생명주기가 비교적 명확합니다.
...
1. 클라이언트는 매칭 시작 버튼 클릭 후 SSE 스트림을 먼저 연결합니다.
2. 서버는 인증된 유저 ID를 기준으로 SSE 연결을 저장하고 `connected` 이벤트를 보냅니다.
3. 클라이언트는 `connected` 수신 후 `joinQueue`를 호출합니다.
4. 매칭 엔진이 두 유저를 원자적으로 큐에서 제거합니다.
5. `MatchFoundService`가 유저 상태와 매칭 세션을 Redis에 저장합니다.
6. `MatchFoundEvent`가 발행됩니다.
7. API 모듈의 Pub/Sub publisher가 `notification:match_found` channel로 메시지를 발행합니다.
8. 모든 API 인스턴스의 subscriber가 메시지를 수신합니다.
9. 각 인스턴스는 자기 `SseConnectionRegistry`에서 두 유저의 SSE 연결을 조회합니다.
10. 연결이 살아 있으면 `match_found` 이벤트를 전송합니다.
11. 연결이 없거나 전송에 실패하면 알림 실패만 로그로 남기고, 매칭 상태는 Redis 기준으로 유지합니다.
...
매칭 시작 요청 전에 호출하는 SSE 연결 API입니다. 인증된 유저 ID를 기준으로 서버가 SSE 연결을 저장합니다.
...
- 사용자가 매칭 대기/응답 흐름을 벗어나면 클라이언트에서 `eventSource.close()`를 호출합니다.
...
- 클라이언트는 매칭 시작 버튼 클릭 후 `joinQueue` 호출 전에 SSE 연결을 먼저 열고, 서버는 연결 유지용 `heartbeat`와 매칭 성사용 `match_found` 이벤트를 전송합니다.
...
1. **SSE 선연결 구조**
   ...
   - 따라서 클라이언트는 `joinQueue` 호출 전에 SSE 연결을 먼저 생성합니다.
