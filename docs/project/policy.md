...
- 배치 게임 중인 유저(`RankSeries.type=PLACEMENT`, `status=IN_PROGRESS`)는 매칭용 tierScore를 **Silver IV 기준 `9`**로 보정해 큐에 진입한다.
- 배치 게임 중인 유저도 일반/Apex 유저와 동일하게 `±1 / ±2 / ±4 / 30초 초과 전체` 매칭 범위 확장 정책을 따른다.
- 배치 유저끼리는 둘 다 tierScore `9`로 큐에 진입하므로 diff `0` 기준으로 즉시 매칭될 수 있다.
- 배치 완료 전 티어 미표시 정책에 따라 match 응답 상대 프로필에서 배치 진행 중 유저는 `Unranked`로 표시한다.
...
- 매칭 알림 SSE 연결은 `match_found` 수신 직후 닫지 않고, 매칭 응답 최종 결과를 받을 때까지 유지한다.
  ...
  - 클라이언트의 매칭 SSE close 여부는 `match_response_result.action`별로 결정한다.
...
- 배치 진행 중 유저는 `RankSeries.type=PLACEMENT`, `status=IN_PROGRESS` 기준으로 식별한다.
- 배치 진행 중 유저는 실제 `UserRankInfo.rank`와 별개로 매칭용 tierScore를 **Silver IV 기준 `9`**로 사용한다.
- 배치 유저끼리는 둘 다 tierScore `9`로 큐에 들어가므로 diff `0` 기준으로 즉시 매칭될 수 있다.
- 배치 유저도 일반/Apex 유저와 동일하게 대기 시간별 매칭 범위 확장 정책을 따른다.
  - `0 ~ 10초`: `9 ± 1`
  - `10초 초과 ~ 20초`: `9 ± 2`
  - `20초 초과 ~ 30초`: `9 ± 4`
  - `30초 초과`: 전체 tierScore 범위 허용
- `30초 정확히`는 기존 `<= 30초` 구간으로 보고 `±4 tierScore`까지만 허용한다.
- 배치 완료 전 티어 미표시 정책에 따라 match 응답 상대 프로필에서 배치 진행 중 유저는 `Unranked`로 표시한다.
- 배치 유저의 실제 rank/LP 정산 정책과 배치 결과 배정 정책은 변경하지 않는다.
