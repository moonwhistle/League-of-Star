# 대규모 매칭 트래픽에서 Redis 기반 상태 공유와 동시성 제어

## 확장 판단 근거

초기에는 단일 API 인스턴스에서도 매칭 기능을 구현할 수 있었다. 하지만 매칭 시스템은 단순 HTTP 요청 처리만으로 끝나지 않고, 매칭 대기열 진입, SSE 연결 유지, match_found 알림 전송, WebSocket 게임방 연결까지 이어진다. 따라서 사용자가 몰리는 상황에서는 API 서버 한 대에 모든 연결과 이벤트 전송을 맡기기보다, 여러 API 인스턴스로 요청과 실시간 연결을 분산할 필요가 있었다.

부하 테스트 기준도 이 판단의 근거가 됐다. steady 테스트에서는 `50 TPS / 5분` 동안 총 `15,000명`의 매칭 진입 요청을 처리했고, 기대값에 가까운 약 `7,500페어`를 성사시켰다. 이때 `joinQueue` p95는 약 `6~15ms`로 안정적이었고, 두 API 인스턴스가 각각 약 `25 req/s`씩 처리했다. 즉, HTTP 진입 지점은 멀티 인스턴스 분산 효과가 있었고 병목으로 보이지 않았다.

실시간 알림 측면에서도 멀티 인스턴스 구조가 필요했다. SSE 부하 테스트에서는 `10,000명` 연결을 유지했고, Redis Pub/Sub 적용 후 `match_found` 이벤트를 `10,000/10,000명`에게 전달했다. 이전 로컬 이벤트 구조에서는 사용자가 연결된 인스턴스와 이벤트가 발생한 인스턴스가 다를 경우 알림 누락 위험이 있었기 때문에, 매칭 이벤트를 Redis Pub/Sub으로 전파하도록 변경했다.

다만 매칭 엔진 자체를 무작정 병렬 실행하지는 않았다. 여러 인스턴스가 같은 Redis 큐를 동시에 스캔하면 동일 유저가 중복 매칭될 수 있기 때문에, 현재 구조에서는 매칭 엔진 실행 구간을 Redisson 전역 락으로 보호했다. 즉, API/SSE/WebSocket 트래픽은 멀티 인스턴스로 분산하고, 최종 매칭 결정 구간은 일관성을 우선해 single active worker에 가깝게 운영한 구조다.

10,000명 burst 테스트에서는 이 선택의 한계도 확인했다. 고정 `leaseTime=5s`를 사용했을 때는 매칭 처리 중 락이 만료되어 다른 인스턴스가 같은 큐를 다시 스캔했고, 원자 제거 실패율이 거의 `100%`까지 증가했다. Redisson watchdog 적용 후 원자 제거 실패율은 `0%`로 개선됐지만, 전체 큐 스캔 구조에서는 스캔 p99가 약 `3.6초`, 매칭 대기 p95가 약 `11초`까지 증가했다. 따라서 현재 구조는 동시성 안정성은 확보했지만, 더 큰 burst를 처리하려면 큐 스캔 범위 축소나 티어별/청크 기반 병렬 처리로 확장해야 한다.
