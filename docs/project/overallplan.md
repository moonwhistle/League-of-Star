...
### 2.2 매칭
- **매칭 큐 진입**: "대전 찾기" 버튼 클릭
- **매칭 알고리즘**: 티어/디비전 기반 유사 실력 상대와 매칭 (±3 디비전 이내)
- **매칭 수락**: 매칭 성사 시 양쪽에 수락/거절 팝업 (10초 타이머)
  - 둘 다 수락 → 바로 게임 시작
  - 한 명이라도 거절/타임아웃 → 수락한 유저는 큐 최우선 복귀 (거절 패널티 없음)
...
| **승급전 방식** | **3판 2선승제** — 2승 시 다음 디비전으로 승급, 2패 시 승급 실패 |
...
| **무승부** | LP 변동 없음 |
| **배치 게임** | 신규 유저는 10판 배치 후 초기 티어 배정 |
...
### Phase 1 — MVP (핵심 플레이 루프)
- [ ] 프로젝트 멀티모듈 세팅 (smite-api / smite-core / smite-infra-redis)
- [ ] DB 세팅 (MySQL)
- [ ] 회원가입 / 로그인 (JWT + 소셜 로그인)
- [ ] 게임 시나리오 엔진 (드래곤 HP 감소 시나리오 생성)
- [ ] WebSocket 기반 실시간 게임 (1v1 강타 싸움)
- [ ] Rewind 판정 로직
- [ ] 매칭 시스템 (Redis 큐 + 매칭 수락 UI)
- [ ] 기본 결과 화면 (승/패/무승부)
- [ ] React 프론트엔드 기본 UI (로그인, 로비, 매칭 수락, 게임, 결과)

### Phase 2 — 랭킹 시스템
- [ ] 티어 & LP 시스템 구현 (티어 차이 기반 LP 증감)
- [ ] 승급전 (3판 2선승제)
- [ ] 티어 기반 매칭 알고리즘 고도화
- [ ] 배치 게임 (10판)
- [ ] 랭킹 리더보드
...
### Phase 3 — 폴리싱 & 확장
- [ ] UI/UX 고도화 (티어 승급 연출, 게임 이펙트)
- [ ] 프로필 페이지 (전적, 승률, 티어 이력)
- [ ] 몬스터 종류 확장 (바론, 전령 등)
- [ ] 강타 데미지 변형 모드 (600/1000/1400)
- [ ] 모바일 반응형
