...
기존 버전과 Outbox 버전에 동일하게 게임 종료 커밋 직후 정산 실행을 중단하는 장애를 1회씩 강제 주입했다. 기존 버전은 실제 구현과 동일하게 `TransactionSynchronization.afterCommit()` callback으로 정산을 직접 호출하고, Outbox 버전은 `AFTER_COMMIT` 즉시 워커 호출을 중단했다.

| 검증 항목 | 기존 afterCommit 직접 호출 | Transactional Outbox |
| --- | ---: | ---: |
| 강제 장애 | 1회 | 1회 |
| 커밋된 FINISHED 게임 | 1 / 1건 | 1 / 1건 |
| 장애 직후 정산 완료 | 0 / 1건 | 0 / 1건 |
| 장애 직후 생성 전적 | 0 / 2건 | 0 / 2건 |
| DB에 남은 정산 작업 | 0건 | INIT 1건 |
| Outbox batch worker 선점 대상 | 0건 | 1건 |
| batch worker 실행 후 Outbox SUCCESS | 해당 없음 | 1 / 1건 |
| batch worker 실행 후 생성 전적 | 0 / 2건 | 2 / 2건 |

강제 장애 시 기존 방식의 직접 정산 callback은 `0/1회` 실행되어 전달 신호가 실제로 유실됐고, 이를 복구할 별도 작업 row도 `0건`이었다. Outbox 방식도 즉시 정산은 `0/1건`이었지만 `INIT` row가 `1/1건` 남았고, scheduler와 동일한 batch worker 실행 후 `SUCCESS 1/1건`, 전적 `2/2건`으로 복구됐다. 이 비교 테스트는 H2와 MySQL 8에서 모두 통과했다.

다만 이 수치는 의도적으로 장애를 주입한 1개 시나리오의 결과이며 운영 환경의 유실 확률을 의미하지 않는다. 기존 버전에도 `FINISHED` 게임과 전적 수를 다시 비교하는 복구 scheduler가 있었으므로 최종 정산이 영구 유실된다고 단정할 수는 없다. 이번 테스트로 확인한 범위는 직접 전달 신호가 유실될 수 있고, 기존에는 게임 테이블 재검색으로 보상했지만 Outbox는 유실된 작업을 명시적인 row 상태로 복구한다는 차이다.
